 ...
 */
gpii.windows.execAndRestartExplorer = function (command, args, options) {
    var defaultOptions = {
        timeout: 5000,
        commandTimeout: 5000,
        pollDelay: 100,
        ...
        if (retries < 0) {
            ...
            if (cmdResErr === false) {
                waitForProcessState(
                    ...
                    function () {
                        gpii.windows.restartExplorer().then(
                            function () {
                                if (cmdResErr === false) {
                                    rPromise.resolve();
                                }
                            ...
                            function (err) {
                                if (cmdResErr === false) {
                                    rPromise.reject({
                                        isError: true,
                                        message: "gpii.windows.execAndRestartExplorer: Explorer restart failed with error '" + err + "'."
                                    });
                                }
                            }
                        );
...
fluid.defaults("gpii.windows.execAndRestartExplorer", {

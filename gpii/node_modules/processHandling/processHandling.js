...
gpii.windows.indentStructString = function (structString) {
    structString.replace("{", "{\r\n")
};

gpii.windows.rmUniqueProcessToString = function (rmUniqueProcess) {
    var string = "";

    string += "dwProcessId: " + rmUniqueProcess.dwProcessId;
    string += ", ";
    string += "ProcessStartTime: " + rmUniqueProcess.ProcessStartTime;

    return string;
};

gpii.windows.rmProcessInfoToObj = function (rmProcessInfo) {
    var appName = gpii.windows.fromWideChar(gpii.windows.arrayToBuffer(rmProcessInfo.strAppName, "TCHAR"));
    var strServiceName = gpii.windows.fromWideChar(gpii.windows.arrayToBuffer(rmProcessInfo.strServiceShortName, "TCHAR"));

    var rmProcessInfoObj = {
        process: rmProcessInfo.process,
        strAppName: appName,
        strServiceShortName: strServiceName,
        applicationType: rmProcessInfo.applicationType,
        appStatus: rmProcessInfo.appStatus,
        tSSessionId: rmProcessInfo.tSSessionId,
        bRestartable: rmProcessInfo.bRestartable
    };

    return rmProcessInfoObj;
};

gpii.windows.RmGetList = function (session) {
    var rm = gpii.windows.restartManager;

    var arraySizeNeededBuf = ref.alloc(gpii.windows.types.UINT);
    var procInfoNumberBuf = ref.alloc(gpii.windows.types.UINT);
    var rebootReasonsBuf = ref.alloc(gpii.windows.types.DWORD);

    ref.set(procInfoNumberBuf, 0, 1, gpii.windows.types.UINT);
    var procArray = new gpii.windows.RM_PROCESS_INFO_ARRAY(1);

    var ret = rm.RmGetList(session, arraySizeNeededBuf, procInfoNumberBuf, procArray, rebootReasonsBuf);

    var requiredSize = arraySizeNeededBuf.deref();
    ref.set(procInfoNumberBuf, 0, requiredSize, gpii.windows.types.UINT);

    procArray = new gpii.windows.RM_PROCESS_INFO_ARRAY(requiredSize);
    var ret = rm.RmGetList(session, arraySizeNeededBuf, procInfoNumberBuf, procArray, rebootReasonsBuf);

    return {ResCode: ret, ProcArray: procArray, RebootReasons: rebootReasonsBuf.deref()};
};
...
gpii.windows.restartProcesses = function (pids, options) {
    ...
    rm.RmShutdown.async(session, RmShutdownOnlyRegistered, ref.NULL, function (err, result) {
        var opres = "ShutdownRes: " + result + "\r\n";
        fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\shutdown.txt', opres, function (err) {
            if (err) {
            } else {
            }
        });
        ...
        if (result) {
            return reject("RmShutdown", result);
            // return gpii.windows.restartProcesses(pids,  options);
        } else {
            if (!options.shutdownOnly) {
                // Re-start the processes again.
                var result = gpii.windows.RmGetList(session);
                var procArray = result.ProcArray;
                var filestr = "";

                if (procArray !== undefined) {
                    for (var i = 0; i < procArray.length; i++) {
                        var string = "Pre: " + JSON.stringify(gpii.windows.rmProcessInfoToObj(procArray[i]));
                        filestr += string;
                    }
                    filestr += "\r\n";
                    fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\proclist.txt', filestr, function (err) {
                        if (err) { } else { }
                    });
                    var rebootReasons = "Pre-RebootReasons: " + result.RebootReasons + "\r\n";
                    fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\rebootReasons.txt', rebootReasons, function (err) {
                        if (err) { } else { }
                    });
                } else {
                    var failCode = "Pre-FailCode: " + result.ResCode + "\r\n";
                    fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\failed-RmList.txt', failCode, function (err) {
                        if (err) { } else { }
                    });
                }

                var ret = rm.RmRestart(session, 0, ref.NULL);
                var res = "RestartRes: " + ret + "\r\n";
                fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\restart.txt', res, function (err) {
                    if (err) { } else { }
                });
                if (ret) {
                    var result = gpii.windows.RmGetList(session);
                    ...
                    if (procArray !== undefined) {
                        for (var i = 0; i < procArray.length; i++) {
                            var string = JSON.stringify(gpii.windows.rmProcessInfoToObj(procArray[i]));
                            filestr += string;
                        }
                        filestr += "\r\n";
                        fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\proclist.txt', filestr, function (err) {
                        ...
                        var rebootReasons = "RebootReasons: " + result.RebootReasons + "\r\n";
                        fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\rebootReasons.txt', rebootReasons, function (err) {
                            if (err) { } else { }
                        });
                    } else {
                        var failCode = "FailCode: " + result.ResCode + "\r\n";
                        fs.appendFile('C:\\Users\\vagrant\\AppData\\Roaming\\gpii\\failed-RmList.txt', failCode, function (err) {
                            if (err) { } else { }
                        });
                    }
...
gpii.windows.closeApplications = function (level, options) {
    ...
    if (levelNum >= levels.low) {
        ...
        return gpii.windows.restartProcesses(pids, {updatePids: true, shutdownOnly: !options.restart});
    }

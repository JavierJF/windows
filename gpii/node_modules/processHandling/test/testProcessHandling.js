...
var rmc = gpii.windows.RestartManagerConstants;

jqUnit.test("Test RmGetList", function () {
    jqUnit.expect(1);
    ...
    // them abruptly.
    var restartableProcesses = gpii.windows.getRestartableProcesses(explorerPids);

    // Put them into a RM_UNIQUE_PROCESS array.
    var processArray = new gpii.windows.RM_UNIQUE_PROCESSES(restartableProcesses.length);
    var index = 0;
    ...
    var sessionKey = new Buffer(rmc.CCH_RM_SESSION_KEY * 2);
    sessionKey.fill(0);

    var sessionRes = rm.RmStartSession(sessionBuf, 0, sessionKey);
    if (sessionRes) {
        jqUnit.fail("RmGetList: Not able to create RestartManager session.");
    }
    ...
    if (ret) {
        jqUnit.fail("RmGetList: Not able to register resources for session.");
    }
    ...
    if (ret) {
        jqUnit.fail("RmGetList: Unable to get processes information.");
    }
    ...
    jqUnit.assertNotEquals("RmGetList: Process array length shouldn't be 0.", procArray.length, 0);
